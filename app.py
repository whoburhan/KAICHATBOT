    ...
    try:
        ...
        if st.session_state.get("uploaded_file_data") and not st.session_state.get("image_processed"):
            ...
            messages.append({"role":"user","parts":[
                "User just uploaded this image. Analyse it only once and relate it to the current query; do not refer back unless explicitly asked. And check in which language is user asking the question and reply in that language only and when they switch language(Use transliteration) you also switch to that same language."
            ]})
