    ...
    try:
        ...
        if image and not st.session_state.get("image_processed"):
            try:
                ...
                st.session_state.chat_history.append(("assistant", "Sorry, I couldn't process that image. Please try another one."))
                return
        
        # Generate response
        response = model.generate_content({"role": "user", "parts": parts})
        reply = response.text or "Sorry, I didn't quite get that — wanna rephrase?"
        ...
        st.session_state.chat_history.append(("assistant", f"Oops, something went wrong: {str(e)}"))
...
def main():
    ...
    # Show sidebar and chat interface
    show_sidebar()
    chat_interface()
